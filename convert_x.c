...
static char	*get_nb(t_printf *env, va_list *ap)
{
	...
	if (env->format.length_modifier == H)
		return (ft_itoa_base((unsigned short)va_arg(*ap, unsigned int), 16, 0));
	if (env->format.length_modifier == L)
		return (ft_ltoa_base(va_arg(*ap, unsigned long), 16, 0));
	if (env->format.length_modifier == LL)
		return (ft_ltoa_base(va_arg(*ap, unsigned long long), 16, 0));
	if (env->format.length_modifier == J)
		return (ft_ltoa_base(va_arg(*ap, unsigned long long), 16, 0));
	if (env->format.length_modifier == Z)
		return (ft_ltoa_base(va_arg(*ap, size_t), 16, 0));
...
int			convert_x(t_printf *env, va_list *ap)
{
	...
	if (env->format.alternate == 1 && env->format.padding != 0)
	...
	if (env->format.alternate == 1 && env->format.padding == 0)
		copy_to_buff(env, "0x", 2);
	if (env->format.alternate == 1)
		padding_precision(env, size_nb - 2 - ft_strlen(s));
	else
